 ...
 * @threshold/provider-respite api`) and this dev server proxies to it, so the page talks to its
...
export default defineConfig({
  ...
  preview: {
    ...
    strictPort: true,
    headers: { 'Origin-Agent-Cluster': '?1' },
    proxy: { '/api': { target: 'http://localhost:6101', changeOrigin: false } },
  },
